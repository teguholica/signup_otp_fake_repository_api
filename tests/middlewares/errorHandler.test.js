...
describe("errorHandler middleware", () => {
    it("should call console.error when NODE_ENV is not test", () => {
        ...
        process.env.NODE_ENV = "development"; // bikin masuk blok
        ...
        expect(spy).toHaveBeenCalledWith(err); // ✅ line 3 ke-cover
    ...
    it("should return 500 and INTERNAL_ERROR when error message not in map", () => {
        const err = new Error(); // tanpa message → undefined
        const req = {};
        const res = {
            status: jest.fn().mockReturnThis(),
            json: jest.fn(),
        ...
        expect(res.status).toHaveBeenCalledWith(500); // ✅ masuk fallback 500
